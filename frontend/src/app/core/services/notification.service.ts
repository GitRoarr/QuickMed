...
import { Notification, NotificationPreferences, NotificationTemplate, NotificationStats } from '../models/notification.model';
...
export class NotificationService {
  ...
  getNotifications(page: number = 1, limit: number = 20, type?: string, priority?: string): Observable<{notifications: Notification[], total: number}> {
    ...
    return this.http.get<{notifications: Notification[], total: number}>(this.apiUrl, { params })
      .pipe(
        tap(response => {
  ...
  createAppointmentNotification(appointmentId: string, type: 'reminder' | 'confirmation' | 'cancellation' | 'reschedule'): Observable<Notification> {
  ...
  createPrescriptionNotification(prescriptionId: string, type: 'ready' | 'refill' | 'expired'): Observable<Notification> {
    const notification: Partial<Notification> = {
      type: 'prescription',
      priority: 'medium',
  ...
  createTestResultNotification(testResultId: string, type: 'available' | 'abnormal' | 'normal'): Observable<Notification> {
    const notification: Partial<Notification> = {
      type: 'test_result',
      priority: 'high',
  ...
  createSystemNotification(title: string, message: string, priority: 'low' | 'medium' | 'high' | 'urgent' = 'medium'): Observable<Notification> {
